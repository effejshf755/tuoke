...
type Pool = { id:number; name:string; productName:string|null; status:string; mode:string; memberLimit:number; startsAt:string|null; endsAt:string|null; createdAt:string; accountId:number|null; accountLabel:string|null; accountStatus:string|null; accountQuotaRemainingPercent:number|null; accountQuotaResetAt:string|null; pendingAccountId:number|null; pendingAccountLabel:string|null; pendingAccountStatus:string|null; pendingAccountQuotaRemainingPercent:number|null; frozenTotalQuotaUnits:number|null; allocationUnits:number|null; usedUnits:number|null; reservedUnits:number|null }
...
export function AdminResourceSubpoolDetailPage() {
  ...
  const bind = useMutation({mutationFn:()=>apiFetch(`/api/admin/resources/subpools/${id}/binding`,{method:'PUT',body:JSON.stringify({codexAccountId:Number(selectedAccount)})}),onSuccess:()=>{toast.success('Codex 账号已选择');setSelectedAccount('');refresh()},onError:(error)=>toast.error(error instanceof Error?error.message:'选择账号失败')})
  const activate = useMutation({mutationFn:()=>apiFetch(`/api/admin/resources/subpools/${id}/activate`,{method:'POST'}),onSuccess:()=>{toast.success('小号池已激活');refresh()},onError:(error)=>toast.error(error instanceof Error?error.message:'激活失败')})
  const clearPending = useMutation({mutationFn:()=>apiFetch(`/api/admin/resources/subpools/${id}/clear-pending-account`,{method:'POST'}),onSuccess:()=>{toast.success('已取消选择账号');setSelectedAccount('');refresh()},onError:(error)=>toast.error(error instanceof Error?error.message:'取消选择失败')})
  ...
  const pendingAvailableUnits = Math.floor(Math.max(0,Math.min(100,Number(pool?.pendingAccountQuotaRemainingPercent||0)))*10_000)
  const quotaShortfall = Math.max(0,Number(pool?.frozenTotalQuotaUnits||0)-pendingAvailableUnits)
  ...
  return <div className="mx-auto w-full max-w-7xl">
    ...
    {detail.isLoading ? <CardSkeleton className="mt-6 h-96" /> : !pool ? <div className="mt-6"><EmptyState title="小号池不存在" /></div> : <div className="mt-6 space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2"><section className="rounded-2xl border bg-card p-5"><h2 className="font-medium">Codex 执行资源</h2>{pool.status==='waiting_resource'?<div className="mt-5">{pool.pendingAccountId&&<div className="mb-4 rounded-lg border bg-muted/35 p-3 text-sm"><div className="flex items-center justify-between gap-3"><div className="font-medium">已选择：{pool.pendingAccountLabel ?? `账号 #${pool.pendingAccountId}`}</div><Button size="sm" variant="ghost" disabled={clearPending.isPending} onClick={()=>clearPending.mutate()}>取消选择</Button></div><div className="mt-1 text-xs text-muted-foreground">状态 {pool.pendingAccountStatus ?? '—'} · 官方剩余 {pool.pendingAccountQuotaRemainingPercent ?? '—'}% · 可用 {quota(pendingAvailableUnits)}</div>{quotaShortfall>0&&<div className="mt-2 text-xs text-destructive">额度不足：商品承诺 {quota(pool.frozenTotalQuotaUnits)}，还差 {quota(quotaShortfall)}。请取消后选择额度充足的账号。</div>}</div>}<AccountSelect value={selectedAccount} setValue={setSelectedAccount} accounts={accounts.data?.accounts??[]}/>{!members.every(member=>member.apiKeyName)&&<p className="mt-2 text-xs text-muted-foreground">尚未创建拼单 Key 的成员可在激活后创建，系统会自动关联到当前套餐。</p>}<div className="mt-4 flex gap-2"><Button variant="outline" disabled={!selectedAccount||bind.isPending} onClick={()=>bind.mutate()}>选择账号</Button><Button disabled={!pool.pendingAccountId||quotaShortfall>0||activate.isPending} onClick={()=>activate.mutate()}><PackageCheck/>激活小号池</Button></div></div>:<><div className="mt-5 grid grid-cols-2 gap-4"><Info label="绑定账号" value={pool.accountLabel??'未绑定'}/><Info label="健康状态" value={pool.accountStatus??'—'}/><Info label="官方剩余额度" value={pool.accountQuotaRemainingPercent==null?'—':`${pool.accountQuotaRemainingPercent}%`}/><Info label="重置时间" value={formatBeijingDateTime(pool.accountQuotaResetAt)}/></div>{['active','paused'].includes(pool.status)&&<div className="mt-5 border-t pt-5"><AccountSelect value={selectedAccount} setValue={setSelectedAccount} accounts={accounts.data?.accounts??[]}/><Button className="mt-3" variant="outline" disabled={!selectedAccount||operation.isPending} onClick={()=>run('change-account',Number(selectedAccount))}><RefreshCw/>确认换绑</Button></div>}</>}</section><section className="rounded-2xl border bg-card p-5"><h2 className="font-medium">额度信息</h2><div className="mt-5 grid grid-cols-2 gap-4"><Info label="小号池总额度" value={quota(pool.allocationUnits)}/><Info label="已使用" value={quota(pool.usedUnits)}/><Info label="已预留" value={quota(pool.reservedUnits)}/><Info label="剩余" value={quota(remaining)}/></div></section></div>
    </div>}
  </div>
}
