  ...
  return <div className="mx-auto w-full max-w-7xl">
    ...
    {query.isLoading ? <p className="py-16 text-center text-sm text-muted-foreground">正在加载 Codex 账号池模型…</p> : query.isError ? <p className="py-16 text-center text-sm text-destructive">模型加载失败</p> : models.length === 0 ? <p className="py-16 text-center text-sm text-muted-foreground">暂无可配置的 Codex 账号池模型</p> : <div className="mt-6 grid gap-5 lg:grid-cols-2">{models.map(model => {
      ...
      return <article key={model.model_id} className="flex min-h-72 flex-col rounded-lg border bg-card p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-5 sm:grid-cols-2"><PriceField label="输入价 / 1M Token" value={draft.input_price_per_million} onChange={value => update(model.model_id, { input_price_per_million: value })} /><PriceField label="输出价 / 1M Token" value={draft.output_price_per_million} onChange={value => update(model.model_id, { output_price_per_million: value })} /><PriceField label="缓存 Token 倍率" value={draft.cached_input_multiplier} step="0.001" onChange={value => update(model.model_id, { cached_input_multiplier: value })} /><PriceField label="计费倍率" value={draft.multiplier} step="0.001" onChange={value => update(model.model_id, { multiplier: value })} /></div>
        <div className="mt-auto pt-6"><Button className="w-full" variant="outline" disabled={save.isPending} onClick={() => save.mutate({ modelId: model.model_id, draft })}><Save />保存</Button></div>
      </article>
    })}</div>}
  </div>
}
