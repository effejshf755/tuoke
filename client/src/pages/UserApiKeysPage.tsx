...
export default function UserApiKeysPage() {
  ...
  return <div className="rounded-3xl border bg-card p-6">
    ...
    <div className="mt-6 space-y-3">{keys.data?.keys.map(key => <div key={key.id} className="rounded-2xl border p-4 text-sm">
      <div className="flex flex-wrap justify-between gap-2"><span className="font-medium">{key.name}</span><code>{key.keyPrefix}...</code><span>{key.keyType === 'codex_pool' ? 'Codex 账号池' : '普通模型'}</span><span>{key.status === 'revoked' ? '已撤销' : key.enabled ? '启用' : '已暂停'}</span></div>
