...
import { Activity, ArrowLeft, BarChart3, Box, CalendarClock, Check, Clock3, CreditCard, Eye, History, PackageCheck, ReceiptText, RefreshCw, ShoppingBag, Users } from 'lucide-react'
...
type UsageRecord = {
  ...
  consumedQuotaUnits: number; inputTokens: number; cachedInputTokens: number; outputTokens: number
  inputMultiplier: number; cachedInputMultiplier: number; outputMultiplier: number
  totalTokens: number; status: string | null; latencyMs: number; createdAt: string
}
type SubscriptionDetail = { subscription: Subscription & { accountPlanType:string|null;accountStatus:string|null;accountQuotaRemainingPercent:number|null;accountQuotaResetAt:string|null;accountQuotaSyncedAt:string|null }; members:Array<{id:number;label:string;isCurrentUser:boolean;allocationUnits:number;usedUnits:number;reservedUnits:number;remainingUnits:number}> }
type UsageAnalytics = { summary:{totalRequests:number;totalInputTokens:number;totalCachedInputTokens:number;totalOutputTokens:number;consumedQuotaUnits:number;avgLatencyMs:number};models:Array<{modelId:string;requests:number;inputTokens:number;cachedInputTokens:number;outputTokens:number;consumedQuotaUnits:number}>;timeline:Array<{day:string;requests:number;totalTokens:number}>;recent:UsageRecord[] }
...
function quota(value: number) { return `${number(value)} 积分` }
...
function ProductCard({ product }: { product: Product }) {
  return <article className="flex min-h-[330px] flex-col rounded-2xl border bg-card p-5 transition-colors hover:bg-card/80">
    <div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><div className="flex size-12 shrink-0 items-center justify-center rounded-lg border bg-white p-2"><img src="/provider-logos/openai.svg" alt="Codex" className="size-full object-contain" /></div><div className="min-w-0"><Badge variant="outline">Codex 月拼单</Badge><h2 className="mt-2 truncate text-lg font-semibold">{product.name}</h2></div></div><div className="shrink-0 text-right"><div className="text-xl font-semibold tabular-nums">{money(product.priceMicro)}</div><div className="text-xs text-muted-foreground">/{cycle(product)}</div></div></div>
...
export function ResourceSubscriptionsPage() {
  ...
  const query = useQuery<{ subscriptions: Subscription[] }>({ queryKey: ['resource-subscriptions'], queryFn: () => apiFetch('/api/resources/subscriptions'), refetchInterval: 10_000 })
  const detail = useQuery<SubscriptionDetail>({queryKey:['resource-subscription-detail',selected],queryFn:()=>apiFetch(`/api/resources/subscriptions/${selected}`),enabled:selected!==null,refetchInterval:10_000})
  const refreshQuota = useMutation({mutationFn:()=>apiFetch<SubscriptionDetail>(`/api/resources/subscriptions/${selected}/refresh-account-quota`,{method:'POST'}),onSuccess:data=>{client.setQueryData(['resource-subscription-detail',selected],data);toast.success('会员账号额度已刷新')},onError:(error:Error)=>toast.error(error.message)})
  return <ResourceShell title="我的 Codex 套餐" description="查看当前有效套餐、个人额度和到期时间。">
    {query.isLoading ? <div className="grid gap-4 md:grid-cols-2">{[1, 2].map(i => <CardSkeleton key={i} className="h-64" />)}</div> : !query.data?.subscriptions.length ? <EmptyState icon={PackageCheck} title="暂无激活套餐" description="订单成团并由管理员激活后会显示在这里。" action={<Link to="/resources/products" className={buttonVariants()}>选购套餐</Link>} /> : <div className="grid gap-4 md:grid-cols-2">{query.data.subscriptions.map(item => { const usedPercent = item.totalQuotaUnits ? Math.min(100, item.usedQuotaUnits / item.totalQuotaUnits * 100) : 0; return <article key={item.subpoolId} className="rounded-2xl border bg-card p-5"><div className="flex items-start justify-between gap-3"><div><Badge variant="secondary"><Check />运行中</Badge><h2 className="mt-3 text-lg font-semibold">{item.productName}</h2></div><span className="text-xs text-muted-foreground">V{item.productVersion}</span></div><div className="mt-6"><div className="flex justify-between text-sm"><span className="text-muted-foreground">剩余额度</span><span className="font-semibold tabular-nums">{quota(item.remainingQuotaUnits)}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-foreground" style={{ width: `${100 - usedPercent}%` }} /></div><div className="mt-2 flex justify-between text-xs text-muted-foreground"><span>已使用 {quota(item.usedQuotaUnits)}</span><span>总计 {quota(item.totalQuotaUnits)}</span></div></div><div className="mt-6 grid grid-cols-2 gap-4 border-t pt-4 text-sm"><div><div className="text-xs text-muted-foreground">到期时间</div><div className="mt-1">{formatBeijingDateTime(item.endsAt)}</div></div><div><div className="text-xs text-muted-foreground">额度重置</div><div className="mt-1">{formatBeijingDateTime(item.quotaResetsAt)}</div></div></div><Button className="mt-5 w-full" variant="outline" onClick={()=>setSelected(item.subpoolId)}><Eye/>查看套餐详情</Button></article> })}</div>}
    <Dialog open={selected!==null} onOpenChange={open=>{if(!open)setSelected(null)}}><DialogPopup className="max-h-[88vh] overflow-y-auto sm:max-w-2xl"><DialogTitle>套餐与会员账号额度</DialogTitle><DialogDescription className="mt-1">仅展示额度和成员使用情况，不公开授权账号身份。</DialogDescription>{detail.isLoading?<CardSkeleton className="mt-5 h-64"/>:detail.data&&<div className="mt-5 space-y-5"><div className="grid gap-3 sm:grid-cols-2"><InfoBlock label="会员类型" value={detail.data.subscription.accountPlanType||'Codex 会员'}/><InfoBlock label="账号状态" value={detail.data.subscription.accountStatus||'—'}/><InfoBlock label="官方剩余额度" value={detail.data.subscription.accountQuotaRemainingPercent==null?'—':`${detail.data.subscription.accountQuotaRemainingPercent}%`}/><InfoBlock label="官方重置时间" value={formatBeijingDateTime(detail.data.subscription.accountQuotaResetAt)}/><InfoBlock label="最近同步" value={formatBeijingDateTime(detail.data.subscription.accountQuotaSyncedAt)}/><InfoBlock label="我的套餐剩余" value={quota(detail.data.subscription.remainingQuotaUnits)}/></div><div><h3 className="text-sm font-medium">拼单成员使用情况</h3><div className="mt-2 divide-y rounded-lg border">{detail.data.members.map(member=><div key={member.id} className="grid grid-cols-[1fr_auto] gap-3 p-3 text-sm"><div><span className="font-medium">{member.label}</span>{member.isCurrentUser&&<Badge className="ml-2" variant="outline">当前用户</Badge>}<div className="mt-1 text-xs text-muted-foreground">已使用 {quota(member.usedUnits)} · 剩余 {quota(member.remainingUnits)}</div></div><div className="text-right"><span className="tabular-nums">{quota(member.remainingUnits)}</span><div className="mt-1 text-xs text-muted-foreground">当前剩余</div></div></div>)}</div></div></div>}<div className="mt-6 flex justify-end gap-2"><DialogClose render={<Button variant="outline"/>}>关闭</DialogClose><Button disabled={refreshQuota.isPending||!detail.data} onClick={()=>refreshQuota.mutate()}><RefreshCw className={refreshQuota.isPending?'animate-spin':''}/>{refreshQuota.isPending?'刷新中':'刷新官方额度'}</Button></div></DialogPopup></Dialog>
  </ResourceShell>
}

function InfoBlock({label,value}:{label:string;value:string}){return <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">{label}</div><div className="mt-1 font-medium">{value}</div></div>}

export function ResourceUsagePage() {
  const query = useQuery<UsageAnalytics>({ queryKey: ['resource-usage-analytics'], queryFn: () => apiFetch('/api/resources/usage/analytics'), refetchInterval:5000 })
  return <ResourceShell title="Codex 使用记录" description="仅显示您自己的套餐调用、权益积分和 Token 消耗。">
    {query.isLoading ? <TableSkeleton rows={7} /> : query.data && <div className="space-y-5">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><UsageMetric icon={Activity} label="套餐调用" value={number(query.data.summary.totalRequests)}/><UsageMetric icon={Box} label="积分消耗" value={quota(query.data.summary.consumedQuotaUnits)}/><UsageMetric icon={BarChart3} label="输入 Token" value={number(query.data.summary.totalInputTokens)}/><UsageMetric icon={BarChart3} label="其中缓存 Token" value={number(query.data.summary.totalCachedInputTokens)}/><UsageMetric icon={BarChart3} label="输出 Token" value={number(query.data.summary.totalOutputTokens)}/></div>
      <section><h2 className="mb-3 font-medium">模型实时概览</h2>{query.data.models.length ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{query.data.models.map(model=><article key={model.modelId} className="rounded-lg border bg-card p-4"><code className="break-all text-sm font-medium">{model.modelId}</code><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><InfoBlock label="调用次数" value={number(model.requests)}/><InfoBlock label="消耗积分" value={quota(model.consumedQuotaUnits)}/><InfoBlock label="输入 Token" value={number(model.inputTokens)}/><InfoBlock label="缓存输入" value={number(model.cachedInputTokens)}/><InfoBlock label="输出 Token" value={number(model.outputTokens)}/></div></article>)}</div> : <div className="rounded-lg border bg-card p-5 text-sm text-muted-foreground">暂无模型调用数据</div>}</section>
      <section><h2 className="mb-3 font-medium">最近调用</h2>{query.data.recent.length ? <div className="overflow-hidden rounded-lg border bg-card"><div className="hidden grid-cols-[1.1fr_1.2fr_.7fr_.8fr_1.3fr] gap-4 border-b bg-muted/30 px-4 py-3 text-xs text-muted-foreground sm:grid"><span>时间</span><span>模型</span><span>状态</span><span>积分</span><span>Token / 倍率</span></div>{query.data.recent.map(record=><div key={record.reservationId} className="grid gap-2 border-b p-4 last:border-0 sm:grid-cols-[1.1fr_1.2fr_.7fr_.8fr_1.3fr] sm:items-center sm:gap-4"><span className="text-sm text-muted-foreground">{formatBeijingDateTime(record.createdAt)}</span><code className="break-all text-sm">{record.modelId||'自动路由'}</code><Badge variant="outline">{record.status||'settled'}</Badge><span className="text-sm tabular-nums">{quota(record.consumedQuotaUnits)}</span><div className="text-sm tabular-nums"><div>普通入 {number(Math.max(0, record.inputTokens-record.cachedInputTokens))} × {record.inputMultiplier}</div><div>缓存入 {number(record.cachedInputTokens)} × {record.cachedInputMultiplier}</div><div>输出 {number(record.outputTokens)} × {record.outputMultiplier}</div><div className="text-xs text-muted-foreground">{number(record.latencyMs)} ms</div></div></div>)}</div> : <EmptyState icon={History} title="暂无使用记录" description="通过套餐调用 Codex 后，记录会显示在这里。" />}</section>
    </div>}
  </ResourceShell>
}

function UsageMetric({icon:Icon,label,value}:{icon:typeof Activity;label:string;value:string}){return <div className="rounded-lg border bg-card p-4"><div className="flex items-center gap-2 text-xs text-muted-foreground"><Icon className="size-4"/>{label}</div><div className="mt-3 text-xl font-semibold tabular-nums">{value}</div></div>}
